    ...
    \node[draw, rectangle, minimum width=10cm, minimum height=1cm] (A) at (0,0) {
      \begin{minipage}{10cm}
        \begin{itemize}
          ...
          \item Choose $q$, a factor of $\phi(p) = p-1$ s.t. $2^{159}<p<2^{160}$
...
\textbf{Passive Evesdropping:} Listening on the common commutication channel between two parties.\\
\subsection*{Active Evesdropping}
  ...
  \begin{tikzpicture}
    ...
    \node[draw, rectangle, minimum width=5cm, minimum height=1cm,anchor=east] (R) at (6,0) {
      \begin{minipage}{5cm}
        ...
        Recieve $B$.\\
        $K_{AB'} = A^{a} \pmod{p} = \alpha^{ab} \pmod{p}$ \\
        $K_{A'B} = A^{a} \pmod{p} = \alpha^{ab} \pmod{p}$ \\
      \end{minipage}
    };
...
\centering   
\end{figure}
Once, such a thing is done, the intruder can not only read the messages sent between them, but also can modify and send messages between them.\\
...
This requires us to send an ID along with message to authenticate. For such an ID, we need to have a centralized agency who produce a certificate. Such an agency is called \textit{Certification Agency(CA)}. When we use https, we see a lock symbol in the browser. Its one kind of a certificate.\\
Now, for a person $A$ to communicate, it asks for a certificate to CA by sending $(A=\alpha^a \pmod{p} , ID_A)$ ($ID_A$ is some ID sent by it like IP address or serial number of processor or something which is unique across world). Now, CA produces a signature($S_A$) on them using its own private key ($K_{private,CA}$) and sends it back along with its public key.
Such a system with CA is called PKI \textit{Public Key Infrastructure}.
\begin{figure}[ht]
  \begin{tikzpicture}
    % First set of text and boxes
    \node[draw, rectangle, minimum width=5cm, minimum height=1cm,anchor=east] (A) at (0,0) {
