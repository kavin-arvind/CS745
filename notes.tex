...
\begin{itemize}
  ...
  \end{align*}
  fermat's little theorem - $a^{\phi(n)} \pmod{n} = a$.
\end{itemize}
...
\begin{center}  
    \huge Lecture - 10 \\
    \Large Topic: Digital Signatures
\end{center}
}
...
\begin{itemize}
  ...
  \item Person $B$ chooses $d$ from ${2,..,p-2}$ and $K_{pub} = \beta = \alpha^d \pmod{p}$ is calculated and sent. This is called Long Term Public Key.
  \item Ephimeral key $K_E$ chosen from ${2,..,p-2}$ such that $gcd(K_E,p-1) = 1$.
  \item Signature is calculated. like $r = \alpha^{K_E} \pmod{p}$ and $s = m-d.r \pmod{p}$. The pair $(r,s)$ is the signature.
  \item \textbf{Verification:} is $m$ same as $\beta^r.\alpha^s \pmod{p}$? If yes, its verified.
\end{itemize}
...
\begin{align*}
  ...
  &= \alpha^{m} \pmod{p}
\end{align*}
\it{seems there is some mistake in Sir's method}\\
...
\begin{center}  
    \huge Lecture - 11 \\
    \Large Topic: Digital Signatures - Elgamal
\end{center}
}

\section*{Digital Signatures - Elgamal}
A wants to send a message to B. A chooses 
\begin{itemize}
  \item p, $\alpha$, 
  \item $K_{pr} = d \in \{2,3,...,p-2\}$.
  \item $K_{pub} = \beta = \alpha^{d} mod p$ ($\beta$ - long term key)
  \item p, $\alpha$, $\beta$ are sent to B
  \item Ephimeral key $K_{E} = d \in \{2,3,...,p-2\}$.
  \item Signature : \\
  r = $\alpha^{K_{E}} mod p$ \\
  S = (m - d.r)$K_{E}^{-1} \mod p$
  \item m,(r,S) sent to B
\end{itemize}
B :
\begin{itemize}
  \item $t = \beta^{r}.r^{S} mod p$
  $t = \alpha^{d.r}.\alpha^{K_{E}(m-d.r)K_{E}^{-1}} mod p$
  $t = \alpha^{m} mod p$ (=valid) 
\end{itemize}

\section*{Digital Signature algorithm}
Digit Signature algorithm reduces the size of the signature from \\
\begin{itemize}
  \item 2048 to 320
  \item 4096 to 422
\end{itemize}
...
\begin{itemize}
  ...
  \item Choose q, a factor of p-1 s.t. $2^{159}<p<2^{160}$
  \item Choose $\alpha$ s.t. $ord(Z_{p}) = q$
  \item $K_{pr} = d \in \{0,1,2,3,...,q-1\}$.
  \item Choose $K_{pub} = \beta = \alpha^{d} mod q$
  \item Public parameters transfered to B are : $K_{pub}$ = p,q,$\alpha,\beta$ %%%%%%%
  \item Ephimeral key $K_{E} \in \{0,1,...,q-1\}$.
  \item Signature : \\
  r = $\alpha^{K_{E}} mod q$ \\
  S = $(SHA(m)-dr) K_{E}^{-1} \mod q$
  \item SHA(m),(r,S) sent to B. (Now the size of the signature is compressed)
\end{itemize}

B:
\begin{itemize}
  \item Verify (SHA(m),(r,S)) = ?
  \item Compute 
  $w = S^{-1} mod q$
  $\mu_{1} = w.SHA(m) mod q$
  $\mu_{2} = w.r mod q$
  $v = (\alpha^{\mu_{1}}.\beta^{\mu_{2}} mod p) mod q$
  \item If $v \equiv \alpha^{SHA(m)}$, then the signature is valid.
  \item   $\mu_{1} = w.SHA(m) mod q$
  $\mu_{1} = S^{-1}.SHA(m) mod q$
  $lll^{ly} \quad \mu_{2} = S^{-1}.r mod q$
\end{itemize}

\[
  r = \alpha^{K_{E}} mod q \\ 
  S = \alpha^{(SHA(m)-d.r)K_{E}^{-1}} mod q \\
  K_{E}.S = (SHA(m)-d.r) mod q \\
  = (\alpha^{\mu_{1}}.\beta^{\mu_{2}} mod p) mod q \\
  = \alpha^{w.SHA(m)}.\alpha^{d(w.r)} mod q \\
  = \alpha^{w.SHA(m) + d(w.r)} mod q \\
  = \alpha^{S^{-1}} \\
  K_{E} = (SHA(m)-d.r).S^{-1} mod q \\
  \alpha^{K_{E}} = \alpha^{(SHA(m)-d.r).S^{-1}} mod q \\
  \alpha^{K_{E}} = (\alpha^{S^{-1}SHA(m)}.\alpha^{S^{-1}.d.r} mod p) mod q \\
  \alpha^{K_{E}} = (\alpha^{S^{-1}(SHA(m) + d.r)} mod p) mod q \\
  \alpha^{K_{E}} = \alpha^{\mu_(1)}.\beta^{\mu_{2}} mod q \\
\]
